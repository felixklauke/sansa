...
public class SimpleSansaServer implements SansaServer {
   ...
   */
  private void handleCommandEnterPassiveMode(FTPRequestContext requestContext,
      FTPRequest ftpRequest) {
    requestContext.setPassiveMode();

    FTPResponse response = new FTPResponse(FTPStatus.ENTERED_PASSIVE_MODE,
        "Sansa is now submissive and passive.");
   ...
   * @param ftpRequest The request.
   */
  private void handleCommandSetTransferType(FTPRequestContext requestContext,
      FTPRequest ftpRequest) {
   ...
   * @param ftpRequest The request.
   */
  private void handleCommandPrintWorkingDir(FTPRequestContext requestContext,
      FTPRequest ftpRequest) {
    File file = requestContext.getCurrentUserWorkingPath();
    FTPResponse response = new FTPResponse(FTPStatus.PATH_CREATED,
  ...
  private void handleUnknownCommand(FTPRequestContext requestContext, FTPRequest ftpRequest) {
    FTPResponse response = new FTPResponse(FTPStatus.UNKNOWN_COMMAND,
        "Sansa never heard about that.");
    requestContext.resume(response);
  }
   ...
   * @param ftpRequest The request.
   */
  private void handleCommandSystemInformation(FTPRequestContext requestContext,
      FTPRequest ftpRequest) {
    FTPResponse response = new FTPResponse(FTPStatus.SYST_STATUS,
        "Motherfuckr this is sansa stark from winterfell.");
    requestContext.resume(response);
  }
  ...
  private void handleCommandUser(FTPRequestContext requestContext, FTPRequest ftpRequest) {
    ...
    FTPResponse response = new FTPResponse(FTPStatus.PASSWORD_NEEDED,
        "Password is need for user '" + userName + "'.");
    requestContext.resume(response);
  }
  ...
  private void handleCommandAuthTLS(FTPRequestContext requestContext, FTPRequest ftpRequest) {
    FTPResponse response = new FTPResponse(FTPStatus.SECURE_CONNECTION_ACCEPTED,
